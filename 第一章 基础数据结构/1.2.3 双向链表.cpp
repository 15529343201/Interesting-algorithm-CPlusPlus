...
template <typename DataType> class DoubleListNode
{
	private:
		...
		//带参数构造函数初始化数据域和指针域
		DoubleListNode(const DataType item,DoubleListNode<DataType> *prior=NULL,
		DoubleListNode<DataType> *next=NULL):m_data(item),m_pprior(prior),m_pnext(next){
